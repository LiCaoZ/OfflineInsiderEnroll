...
if %build% LSS 17763 (
    echo =============================================================
    echo 此脚本仅适配于 Windows 10 v1809 及以上版本。
    echo =============================================================
    echo.
    pause
    goto :EOF
...
REM If not elevated, restart as admin
...
:CHOICE_MENU
cls
title Windows 预览体验计划离线注册 v%scriptver%
set "choice="
echo.
echo 0 - 金丝雀 Canary 渠道
echo 1 - 开发人员 Dev 渠道
echo 2 - Beta 渠道
echo 3 - Release Preview 渠道
echo.
echo 4 - 停止接收预览体验构建
echo 5 - 退出而不做任何更改
echo.
set /p choice="请做出你的选择: "
echo.
...
if %build% LSS 21990 reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\WindowsSelfHost\UI\Strings" /f /t REG_SZ /v StickyXaml /d "<StackPanel xmlns="^""http://schemas.microsoft.com/winfx/2006/xaml/presentation"^""><TextBlock Style="^""{StaticResource BodyTextBlockStyle }"^"">此设备已由 Windows 预览体验计划离线注册脚本 v%scriptver% 注册至 Windows 预览体验计划。如果你希望修改注册设置或者是停止接收预览体验构建，请使用该脚本。<Hyperlink NavigateUri="^""https://blog.licaoz.com/post/windows-insider-enroll/"^"" TextDecorations="^""None"^"">了解更多</Hyperlink></TextBlock><TextBlock Text="^""已应用的配置"^"" Margin="^""0,20,0,10"^"" Style="^""{StaticResource SubtitleTextBlockStyle}"^"" /><TextBlock Style="^""{StaticResource BodyTextBlockStyle }"^"" Margin="^""0,0,0,5"^""><Run FontFamily="^""Segoe MDL2 Assets"^"">&#xECA7;</Run> <Span FontWeight="^""SemiBold"^"">%Fancy%</Span></TextBlock><TextBlock Text="^""渠道：%Channel%"^"" Style="^""{StaticResource BodyTextBlockStyle }"^"" /><TextBlock Text="^""内容：%Content%"^"" Style="^""{StaticResource BodyTextBlockStyle }"^"" /><TextBlock Text="^""遥测配置提醒"^"" Margin="^""0,20,0,10"^"" Style="^""{StaticResource SubtitleTextBlockStyle}"^"" /><TextBlock Style="^""{StaticResource BodyTextBlockStyle }"^"">Windows 预览体验计划需要你将遥测数据收集更改为 <Span FontWeight="^""SemiBold"^"">完整</Span>。你可以在 <Span FontWeight="^""SemiBold"^"">遥测与反馈设置</Span> 验证或者修改当前配置。</TextBlock><Button Command="^""{StaticResource ActivateUriCommand}"^"" CommandParameter="^""ms-settings:privacy-feedback"^"" Margin="^""0,10,0,0"^""><TextBlock Margin="^""5,0,5,0"^"">打开遥测与反馈设置</TextBlock></Button></StackPanel>"
...
echo "StickyMessage"="{\"Message\":\"设备已本地注册至 Windows 预览体验计划\",\"LinkTitle\":\"\",\"LinkUrl\":\"\",\"DynamicXaml\":\"^<StackPanel xmlns=\\\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\\\"^>^<TextBlock Style=\\\"{StaticResource BodyTextBlockStyle }\\\"^>此设备已由 Windows 预览体验计划离线注册脚本 v%scriptver% 注册至 Windows 预览体验计划。如果你希望修改注册设置或者是停止接收预览体验构建，请使用该脚本。^<Hyperlink NavigateUri=\\\"https://blog.licaoz.com/post/windows-insider-enroll/\\\" TextDecorations=\\\"None\\\"^>了解更多^</Hyperlink^>^</TextBlock^>^<TextBlock Text=\\\"已应用的配置\\\" Margin=\\\"0,20,0,10\\\" Style=\\\"{StaticResource SubtitleTextBlockStyle}\\\" /^>^<TextBlock Style=\\\"{StaticResource BodyTextBlockStyle }\\\" Margin=\\\"0,0,0,5\\\"^>^<Run FontFamily=\\\"Segoe MDL2 Assets\\\"^>^&#xECA7;^</Run^> ^<Span FontWeight=\\\"SemiBold\\\"^>%Fancy%^</Span^>^</TextBlock^>^<TextBlock Text=\\\"渠道：%Channel%\\\" Style=\\\"{StaticResource BodyTextBlockStyle }\\\" /^>^<TextBlock Text=\\\"内容：%Content%\\\" Style=\\\"{StaticResource BodyTextBlockStyle }\\\" /^>^<TextBlock Text=\\\"遥测配置提醒\\\" Margin=\\\"0,20,0,10\\\" Style=\\\"{StaticResource SubtitleTextBlockStyle}\\\" /^>^<TextBlock Style=\\\"{StaticResource BodyTextBlockStyle }\\\"^>Windows 预览体验计划需要你将遥测数据收集更改为 ^<Span FontWeight=\\\"SemiBold\\\"^>完整^</Span^>。你可以在 ^<Span FontWeight=\\\"SemiBold\\\"^>遥测与反馈设置^</Span^> 验证或者修改当前配置。^</TextBlock^>^<Button Command=\\\"{StaticResource ActivateUriCommand}\\\" CommandParameter=\\\"ms-settings:privacy-feedback\\\" Margin=\\\"0,10,0,0\\\"^>^<TextBlock Margin=\\\"5,0,5,0\\\"^>打开遥测与反馈设置^</TextBlock^>^</Button^>^</StackPanel^>\",\"Severity\":0}"
echo.
)>"%SystemRoot%\oie.reg"
...
:ASK_FOR_REBOOT
set "choice="
echo 为了应用你的更改，我们需要重新启动计算机。
set /p choice="你希望现在重新启动你的计算机吗？ (y/N) "
